...
std::string get_error_response(int code, method_type method_type,
                               server *server);
std::string make_autoindex_body(std::string root, std::string path,
                                std::string cur_url);
std::string make_header_response(int code_num, method_type method_type,
...
config_path_search match_against_config_path(server &server,
                                             UserRequestInfo &req) {
  ...
  for (size_t i = 0; i < server.loc_method.size(); i++) {
    ...
    cur_size_read = 0;
    total_size_read = 0;
    std::vector<std::string> split_loc_meth =
        my_strsplit(server.loc_method[i].path, '/');
  ...
  for (size_t i = 0; i < server.loc_method.size(); i++) {
    ...
    for (j = 0; j < req.subdomains.size(); j++) {
      cur_size_read = std::strcmp(server.loc_method[i].path.c_str(), req.subdomains[j].c_str());
      if (cur_size_read == 0) {
        break; // super break
      }
  ...
  // ! this so that he check the root after trying everything else (need to be
  // better later)
...
// std::string make_header_response(int status_code, int content_type, int
// content_lenght) {
...
std::string handle_single_connetion(std::string &root, std::string html_page) {
  std::string response;
...
std::string handle_single_redirection(int ret_val, std::string &redirection) {
  std::string response;
...
std::string check_server_root_files(server &server,
...
bool is_surplus_valid_file(std::string sur_plus) {
  if (sur_plus.empty())
    return false;
  sur_plus.pop_back();
...
void handle_get_request(int client_fd, server &server,
                        UserRequestInfo &user_request) {
  std::string response;
  ...
  if (config_path_index == -1) {
    ...
    if (server.root.empty()) {
      response = get_error_response(404, GET, &server);
        ...
        response = handle_single_redirection(server.redirect.retValue,
                                             server.redirect.path);
    } else {
      response =
          handle_single_connection_no_subdomain(server.root, server.index);
    }
      ...
      if (!server.loc_method[config_path_index].redirection.empty) {
      ...
      else if (is_method_allowed(user_request,
                                 server.loc_method[config_path_index]) != 0) {
        ...
        response = handle_single_connetion(server.root, server.index);
      else if (!server.loc_method[config_path_index].index.empty() &&
               config_parsed.surplus.empty())
        response = handle_single_connetion(
            server.root, server.loc_method[config_path_index].index);
      else if (is_surplus_valid_file(config_parsed.surplus)) {
        ...
        if (f.good()) {
          ...
          response =
              make_header_response(200, GET, config_parsed.surplus, body.size());
...
size_t find_cgi_path(server &server) {
  for (size_t i = 0; i < server.loc_method.size(); i++) {
    if (!server.loc_method[i].cgi_execute.empty() &&
        !server.loc_method[i].cgi_path.empty())
      return i;
  }
  return -1;
}
...
char **make_arg_cgi(const std::string &cgi_asked, const std::string &root,
  ...
  // if (end_script_name != std::string::npos && end_script_name + 1 <
  // cgi_asked.size()) {
...
// std::string &get_content_type_from_header_request(std::map<std::string,
...
std::map<std::string, std::string> make_env(server &server, char **exe_arg,
                                            UserRequestInfo &user_request) {
  ...
  env["QUERY_STRING"] =
      user_request.subdomains.back().substr(end_script_name + 1);
  // env[""] = ;
...
void handle_cgi_request(int client_fd, server &server,
                        UserRequestInfo &user_request) {
  int index_config_cgi = find_cgi_path(server);
  std::string response;
  std::string body;
  ...
  if (pid == 0) { // Child process
    ...
    // setenv("QUERY_STRING",
    // user_request.subdomains.back().substr(end_script_name + 1).c_str(), 1);
    // setenv("REQUEST_METHOD", "GET", 1);
    // setenv("CONTENT_TYPE", "text/html; charset=UTF-8", 1);
    // setenv("CONTENT_LENGTH",
    // std::to_string(user_request.content_length).c_str(), 1);
    ...
    // char **env = nullptr;
    char **env = map_to_array(make_env(server, argv, user_request));
    ...
    if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
      ...
      close(pipefd[0]);
      close(pipefd[1]);
      response = make_header_response(200, is_method_ask(user_request),
                                      "python.html", body.size());
      response.append(body);
    } else {
      close(pipefd[0]);
      close(pipefd[1]);
...
void handle_connection(int client_fd, running_server *server) {
  ...
  if (!user_request.subdomains.empty() &&
    ...
    handle_get_request(client_fd, server->subdomain[config_server_index],
             ...
             user_request.body.find("DELETE") != std::string::npos) {
    handle_del_request(client_fd, server->subdomain[config_server_index],
                       user_request);
  } else {
...
// char ** make_arg_cgi(std::string cgi_asked, std::string root, std::string
// our_cgi_path, std::string cgi_path) {
